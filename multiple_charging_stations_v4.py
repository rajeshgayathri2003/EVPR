...
def find_nash_equilibrium(grid_price, 
    ...
    for station in range(stations):
        ...
        combined_profits.append(profit_variation)
        combined_demand.append(demand_variation)
        combined_sell_price.append(cs_sell_price_variation)
        combined_omega.append(omega_variation)
        
        print("valuessss")
        print(combined_profits)
        print(combined_demand)
        print(combined_sell_price)
            
        combined_plot_price(profit_variation, omega_variation, cs_sell_price_variation, demand_variation, nash)
        
    combined_plot_price_three(combined_profits, combined_omega, combined_sell_price, combined_demand, nash)
...
def combined_plot_price_three(profit_variation_list, omega_variation_list, cs_sell_price_variation_list, demand_variation_list, nash):
    fig, ax1 = plt.subplots(figsize=(10, 6))  # Increase figure size

    # Define labels and colors
    station_labels = ["Charging Station 4", "Charging Station 7", "Charging Station 20"]
    colors = ['b', 'g', 'r']  # Blue, Green, Red

    ax2 = ax1.twinx()  # Create secondary y-axis once

    legend_handles = []  # To store one handle per station

    # Plot for each charging station
    for i in range(3):
        price_plot = cs_sell_price_variation_list[i][1:, 0, 0]
        profit_plot = profit_variation_list[i]
        demand_plot = demand_variation_list[i][:, 0, 0]

        # Plot profit on ax1
        ax1.plot(price_plot, profit_plot, color=colors[i], linestyle='-')

        # Plot demand on ax2
        ax2.plot(price_plot, demand_plot, color=colors[i], linestyle='--')

        # Add just one handle per station (use solid line as representative)
        handle = mlines.Line2D([], [], color=colors[i], label=station_labels[i])
        legend_handles.append(handle)

    # Nash equilibrium vertical line
    nash_handle = ax1.axvline(x=nash, color='m', linestyle='dotted', linewidth=2, label='Nash Equilibrium')

    # Axis labels
    ax1.set_xlabel("Selling Price of Electricity to EVs")
    ax1.set_ylabel("")
    ax1.text(-0.08, 0.45, "Profit (₹)", color="black", fontsize=14, transform=ax1.transAxes,
             va='center', ha='center', rotation=90)
    ax2.set_ylabel("Demand (kWh)", color='black', fontsize=14)
    ax2.tick_params(axis='y', labelcolor='black')

    # Title and grid
    fig.suptitle("Nash Equilibrium across Charging Stations", fontsize=18)
    ax1.grid()

    # Append Nash line handle to legend
    legend_handles.append(nash_handle)

    # Place legend outside the plot on the right
    ax1.legend(handles=legend_handles, loc="best", fontsize=10)

    plt.tight_layout()  # Make space for legend
    plt.show()
...
#     # Plot for each charging station
#     for i in range(3):
#         price_plot = cs_sell_price_variation_list[i][1:, 0, 0]  # Skip first element like original code
#         profit_plot = profit_variation_list[i]
#         demand_plot = demand_variation_list[i][:, 0, 0]

#         # Plot profit on primary y-axis
#         ax1.plot(price_plot, profit_plot, label=f"{station_labels[i]} - Profit", color=colors[i], linestyle='-')

#         # Plot demand on secondary y-axis
#         ax2 = ax1.twinx() if i == 0 else ax2  # Only create once
#         ax2.plot(price_plot, demand_plot, label=f"{station_labels[i]} - Demand", color=colors[i], linestyle='--')
...
#     # Axis labels
#     ax1.set_xlabel("Selling Price of Electricity to EVs")
#     ax1.set_ylabel("")  # Left blank to use custom text annotation
#     ax1.text(-0.1, 0.45, "Profit (₹)", color="black", fontsize=14, transform=ax1.transAxes, 
#              va='center', ha='center', rotation=90)
#     ax2.set_ylabel("Demand (kWh)", color='black', fontsize=14)
#     ax2.tick_params(axis='y', labelcolor='black')
...
#     # Plot for each charging station
#     for i in range(3):
#         price_plot = cs_sell_price_variation_list[i][1:, 0, 0]  # Skip first element
#         profit_plot = profit_variation_list[i]
#         demand_plot = demand_variation_list[i][:, 0, 0]
...
#     # Combine legends and move outside the plot
#     lines_1, labels_1 = ax1.get_legend_handles_labels()
#     lines_2, labels_2 = ax2.get_legend_handles_labels()
#     ax1.legend(
#         lines_1 + lines_2, labels_1 + labels_2,
#         loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=10, borderaxespad=0.
#     )
